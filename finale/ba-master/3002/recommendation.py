...
def get_similar_users(user_id):
    ...
    current_user_movies = Review.query.filter_by(user_id=user_id).filter(Review.rating >= 7).all()
    ...
    for movie_id in current_user_movie_ids:
        # Find other users who rated the same movie >= 7
...
def get_personalized_recommendations(user):
    ...
    - Content-based filtering (recommendations for the last 2 movies rated >7).
    ...
    # Fetch the last 2 movies rated >7 by the user
